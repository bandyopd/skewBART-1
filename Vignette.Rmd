...
library(ggplot2)
load("~/Documents/alpha_soft_fine.RData")
load("~/Documents/alpha_skew_fine.RData")
...
ggplot(df, aes(x = alpha, y=LPML, group=method)) +
  ...
  geom_smooth(aes(ymin = lb, ymax = ub, fill = method, colour = method),linetype=0, stat = "identity",alpha=0.1) +
  theme_bw() + theme(legend.position="bottom",axis.text = element_text(face="bold"), legend.title = element_blank())+
  xlab(expression(alpha)) + scale_colour_manual(values=custom.col)+
...
```{r, echo=FALSE}
kable(cbind(lambda, rho, multi_skew, multi))
```
